  ...
  useEffect(() => {
    const fetchHistory = async () => {
      ...
      try {
        ...
        setIsHistoryLoading(false); // <-- Stop loading history
      }
    };
    fetchHistory();
  }, []);
  ...
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-zinc-900 text-gray-900 dark:text-gray-100 transition-colors duration-300 font-sans">
      ...
      <main className="container mx-auto max-w-7xl p-4 md:p-8">
        ...
        <AnimatePresence>
          {error && (
            <motion.div /* ... */ className="mb-6 p-4 bg-red-100 dark:bg-red-900/50 border border-red-500 text-red-700 dark:text-red-300 rounded-lg shadow-md flex items-center gap-3">
         ...
         <AnimatePresence>
           {deleteError && (
             <motion.div /* ... */ className="mb-6 p-4 bg-orange-100 dark:bg-orange-900/50 border border-orange-500 text-orange-700 dark:text-orange-300 rounded-lg shadow-md flex items-center gap-3">
...
function Header({ title, theme, onToggleTheme }) { /* ... */
    return (
    <motion.nav /* ... */ className="sticky top-0 z-50 w-full bg-white/70 dark:bg-zinc-800/70 backdrop-blur-lg border-b border-gray-200 dark:border-zinc-700">
      <div className="container mx-auto max-w-7xl px-4 md:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <motion.button /* ... */ onClick={onToggleTheme} className="p-2 rounded-full text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-zinc-700 transition-colors" aria-label="Toggle theme">
            <AnimatePresence mode="wait">{theme === 'light' ? (<motion.div key="moon" /* ... */><Moon size={20} /></motion.div>) : (<motion.div key="sun" /* ... */><Sun size={20} /></motion.div>)}</AnimatePresence>
          </motion.button>
        </div>
      </div>
    </motion.nav>
...
function InputSection({ inputText, onInputChange, onAnalyze, isLoading }) { /* ... */
   return (
    <motion.div /* ... */ className="p-6 bg-white/70 dark:bg-zinc-800/70 backdrop-blur-lg border border-gray-200 dark:border-zinc-700 rounded-xl shadow-lg">
                ...
                <li key={index} className="text-sm">
                  ...
                  {source.uri ? (
                     <a href={source.uri} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline">
                       {source.title || new URL(source.uri).hostname} {/* Show title or hostname */}
                     </a>
                  ) : (
...
function HistorySection({ history, isLoading, onItemClick, onDeleteItem }) {
  return (
    <motion.div /* ... */ className="p-6 bg-white/70 dark:bg-zinc-800/70 backdrop-blur-lg border border-gray-200 dark:border-zinc-700 rounded-xl shadow-lg h-full flex flex-col">
...
function HistoryItem({ item, index, onClick, onDelete }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex-grow cursor-pointer" onClick={handleDetailsClick}> {/* Make info section clickable */}
