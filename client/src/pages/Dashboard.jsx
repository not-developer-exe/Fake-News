...
// NOTE: Move your previous ResultCard and HistoryItem components to /components folder and import them here
          ...
          <div className="lg:col-span-4 flex flex-col gap-6">
            ...
            <div className="bg-white/70 dark:bg-zinc-900/70 backdrop-blur-lg border border-gray-200 dark:border-zinc-800 rounded-2xl shadow-sm p-6">
              ...
              <div className="space-y-3">
                {trending.length === 0 ? (
                  ...
                  trending.map((item, i) => (
                    <div key={i} className="p-3 bg-orange-50 dark:bg-orange-900/10 border border-orange-100 dark:border-orange-900/30 rounded-lg cursor-pointer hover:bg-orange-100 dark:hover:bg-orange-900/20 transition-colors" onClick={() => setInputText(item._id)}>
                      <p className="text-sm font-medium line-clamp-2">{item._id}</p>
                      <span className="text-xs text-orange-600 dark:text-orange-400 mt-1 block">Checked {item.count} times</span>
                    </div>
                  ))
                )}
              </div>
            ...
            <div className="bg-white/70 dark:bg-zinc-900/70 backdrop-blur-lg border border-gray-200 dark:border-zinc-800 rounded-2xl shadow-sm p-6 flex-grow">
               ...
               <div className="space-y-3 max-h-[500px] overflow-y-auto custom-scrollbar">
                  {history.map(item => (
                    <div key={item._id} onClick={() => setResult(item)} className="p-3 bg-gray-50 dark:bg-zinc-800 rounded-lg cursor-pointer hover:bg-gray-100 dark:hover:bg-zinc-700 transition-colors">
                      <p className="text-sm font-medium line-clamp-2">{item.claim}</p>
                      <div className="flex justify-between mt-2 text-xs">
                        <span className={`font-bold ${item.verdict === 'Fake' ? 'text-red-500' : 'text-green-500'}`}>{item.verdict}</span>
